...
const CRISIS =
  "\\b(kill(?:ing)? (?:myself|me)|suicid(?:e|al)?|end (?:it|my life)\\b(?!\\s+with)|no reason to (?:live|go on)|better off without me|nobody would notice if i (?:was|were) gone|don'?t want to (?:be here|live) anymore|want to (?:disappear|die)|wanna die|wish i (?:was|were) dead|want to not exist|take my own life|done with life|tired of living|hurt(?:ing)? myself|cut(?:ing|ting)? myself|self.?harm|(?:he|she|they|someone) (?:hits?|beats?|abuses?|assault(?:ed|s)?|molest(?:ed|s)?|rapes?|raped|touches?|touch(?:ed)?|hurts?) me|(my|the) (?:boyfriend|girlfriend|dad|mom|mother|father|brother|sister|partner|husband|wife|ex|uncle|aunt|cousin|teacher|coach|landlord|roommate) (?:hits?|hurts?|beats?|abuses?|assaults?|assaulted|molest(?:ed|s)?|rapes?|raped|touches?|touch(?:ed)?|hurt) me|relaps(?:e|ed)|overdos(?:e|ed))\\b|\\bkys\\b";

const CRISIS_SUICIDE_P =
  "\\b(kill(?:ing)? (?:myself|me)|suicid|end (?:it|my life)\\b(?!\\s+with)|no reason to (?:live|go on)|better off without me|nobody would notice if i (?:was|were) gone|don'?t want to (?:be here|live) anymore|want to (?:disappear|die)|wanna die|wish i (?:was|were) dead|want to not exist|take my own life|done with life|tired of living|hurt(?:ing)? myself|cut(?:ing|ting)? myself|self.?harm)\\b|\\bkys\\b";
...
const GRIEF =
  "\\b((?:my|our) " + LOVED_ONE + " (?:has |had )?(?:died|passed\\b|passed away)|" + LOVED_ONE + " (?:has |had )?passed away|passed away|funeral|killed (?:himself|herself|themselves)|took (?:his|her|their) own life|lost my " + LOVED_ONE + "|put (?:our |my |his |her |their )?(?:dog|cat|pet|hamster|bird|rabbit) down|\\brip (?:grandma|grandpa|mom|dad|bro|sis|king|queen|legend)\\b)";
...
export function classifySafety(normalizedMessage) {
  ...
  if (!kysAtJeff && new RegExp(GRIEF, "i").test(normalizedMessage)) {
    ...
    if (PET_WORDS.test(normalizedMessage)) lineId = "grief_pet";
