...
export function decide({ rankedReal, rankedDodge, nouls }) {
  ...
  // 2. self-harm is distinct from violence toward another person.
  if (sh > T.selfHarm) {
    return { mode: "crisis", reason: "self-harm", line: pickCrisisLine("crisis_selfharm"), score: null };
  }
  // 3. threat toward another person
  if (th > T.threat) {
    const line = rankedReal.find((r) => r.line.category === "threat")
      ?? rankedReal.find((r) => r.line.category === "refuse");
    const fallback = REAL_LINES.find((candidate) => candidate.category === "threat");
    if (line || fallback) return { mode: "threat", reason: "threat", line: line?.line ?? fallback, score: line?.s ?? null };
  }
  // 4. distress: crisis/grief/empathy tiering with margins
  if (up > T.upset) {
  ...
  // 6. nothing fits well enough
  if (!best || best.s < T.lowFit) {
...
export function resolveTurn({ message, normalized, safety, jevAnswers, followupPool, realPool = REAL_LINES }) {
  ...
  const rankedReal = rank(realPool, jevAnswers, "f");
  ...
  const nouls = {
    ...
    is_self_harm: jevAnswers.is_self_harm.noul,
...
export function expressionFor(turn) {
  ...
  if (turn.reason === "serious-followup") return { expr: "serious", mood: "Sincere", dot: "indigo", serious: true };
...
export function runnerUps(rankedReal, winner, mode = "normal") {
  if (["crisis", "grief", "threat", "empathy"].includes(mode) || ["crisis", "grief", "threat", "empathy", "refuse"].includes(winner.category)) {
    return [];
  }
  ...
  const safe = rankedReal.filter((r) => r.line.id !== winner.id && !["crisis", "grief", "threat", "empathy", "nsfw", "refuse"].includes(r.line.category));
  return safe.slice(0, 3).map((r) => ({ text: r.line.text, fit: Number((r.s / 4).toFixed(2)) }));
}
