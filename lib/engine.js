...
const LITERAL_REACTION_IDS = new Set([
  "reactions_lol",
  "reactions_lmao",
  "reactions_ok",
  "reactions_question_mark",
  "reactions_no",
  "reactions_yes",
  "reactions_bruh",
  "reactions_huh",
...
const MODEL_REAL_LINES = REAL_LINES.filter((line) => !LITERAL_REACTION_IDS.has(line.id));

const REACTION_INPUTS = new Map([
  ["lol", { key: "laugh", lineId: "reactions_lol" }],
  ["haha", { key: "laugh", lineId: "reactions_lol" }],
  ["hahaha", { key: "laugh", lineId: "reactions_lol" }],
  ["lmao", { key: "laugh", lineId: "reactions_lmao" }],
  ["rofl", { key: "laugh", lineId: "reactions_lmao" }],
  ["ok", { key: "ok", lineId: "reactions_ok" }],
  ["okay", { key: "ok", lineId: "reactions_ok" }],
  ["k", { key: "ok", lineId: "reactions_ok" }],
  ["no", { key: "no", lineId: "reactions_no" }],
  ["nope", { key: "no", lineId: "reactions_no" }],
  ["nah", { key: "no", lineId: "reactions_no" }],
  ["yes", { key: "yes", lineId: "reactions_yes" }],
  ["yep", { key: "yes", lineId: "reactions_yes" }],
  ["yeah", { key: "yes", lineId: "reactions_yes" }],
  ["bruh", { key: "bruh", lineId: "reactions_bruh" }],
  ["bro", { key: "bruh", lineId: "reactions_bruh" }],
  ["huh", { key: "huh", lineId: "reactions_huh" }],
  ["hm", { key: "hmm", lineId: "reactions_hmm" }],
  ["hmm", { key: "hmm", lineId: "reactions_hmm" }],
  ["same", { key: "same", lineId: "reactions_same" }],
  ["fr", { key: "fr", lineId: "reactions_fr" }],
  ["ugh", { key: "ugh", lineId: "reactions_ugh" }],
]);

export function lowEffortReaction(message) {
  const normalized = normalize(message);
  if (/^[?？]+$/u.test(normalized)) {
    return { key: "question-mark", lineId: "reactions_question_mark" };
  }
  if (/^🔥+$/u.test(normalized)) {
    return { key: "fire", lineId: "reaction_fire" };
  }
  const bare = normalized.replace(/^[\p{P}\p{S}]+|[\p{P}\p{S}]+$/gu, "").trim();
  return REACTION_INPUTS.get(bare) ?? null;
}

export function consecutiveLowEffortCount(message, history) {
  const reaction = lowEffortReaction(message);
  if (!reaction) return 0;
  let count = 1;
  for (let index = history.length - 1; index >= 0; index--) {
    if (lowEffortReaction(history[index]?.user)?.key !== reaction.key) break;
    count++;
  }
  return count;
}
...
export async function runEngineTurn({ message, history = [], repeatCount = 0, callJev }) {
  const t0 = Date.now();
  const normalized = normalize(message);
  ...
  const reaction = lowEffortReaction(message);
  if (reaction) {
    const repeats = Math.max(repeatCount, consecutiveLowEffortCount(message, history));
    const lineId = repeats >= 2 ? "reactions_repeat" : reaction.lineId;
    const line = REAL_LINES.find((candidate) => candidate.id === lineId);
    if (!line) throw new Error(`Missing reaction line: ${lineId}`);
    const turn = { mode: "normal", reason: repeats >= 2 ? "repeat-filler" : "exact-reaction", line, score: null };
    return {
      turn,
  ...
  const pool = followupMode ? FOLLOWUP_LINES : MODEL_REAL_LINES;
  ...
  const turn = resolveTurn({
    ...
    realPool: followupMode ? undefined : pool,
  });
